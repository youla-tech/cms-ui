<template>
  <a-card :bordered="false">
    ...
    <a-row :gutter="12">
      <div v-for="(dynamic, index) in dynamicFields" :key="index" style="margin-bottom: 10px;">
        <a-col
          :xl="{ span: 12 }"
          :lg="{ span: 12 }"
          :md="{ span: 12 }"
...
export default {
  ...
  methods: {
    ...
    handleFileChange(e, fieldName) {
      this.fileObjField[fieldName].fileList = e.fileList
      if (e.file.status==='done' && e.file.response.code === 0) {
        this.fileObjField[fieldName].fileObj = e.file.response.res
      }
    },
    attachFileChange(e) {
      this.attachFiles = e.fileList
    ...
    getParams() {
      ...
      if (fileAttrbute.length > 0) {
        fileAttrbute.forEach(function(item, index) {
          if (fileObjField[item].fileList.length > 0) {
            if(item.indexOf(extendFieldKey) === 0){
                let reItemName= item.replace(extendFieldKey,"");
				        newParam.extendParam[reItemName] = fileObjField[item].fileObj
			      }else{
                newParam[item] = fileObjField[item].fileObj
            }
          }
        })
      }
      ...
      if (this.attachFiles.length > 0) {
        const attachs = []
        this.attachFiles.forEach(function(item, index) {
          if (item.response && item.response.code===0) {
             let attach= Object.assign({
               sysResourceId:item.response.res.id
             },item.response.res)
             attachs.push(attach)
          }
        })
