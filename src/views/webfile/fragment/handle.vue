          ...
          <a-col :xl="{span: 11}" :lg="{span: 8}" :md="{span: 12}">
            <a-form-item label="数据条数" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number
                style="width: 100%;"
                :min="1"
                v-decorator="['size', {rules: [{required: true, message: '请输入'}],initialValue:0}]"
              />
            </a-form-item>
          </a-col>
        </a-row>
...
<script>
import { save, getById, update } from "@/api/cms/webfile/fragment.js";
    ...
    add(fileItem) {
      ...
      setTimeout(function() {
        ...
        if (fileItem.filePath) {
          pathName =
            fileItem.filePath.indexOf("\\") === 0
              ? fileItem.filePath.substring(1, fileItem.filePath.length - 1)
              : fileItem.filePath;
        }
        that.form.setFieldsValue({
          filePath: fileItem.filePath ? fileItem.filePath : ""
        });
